...
function SignUp({ handleCloseSignupModal, setLoginModal }) {
  ...
  const handleInputValue = (key) => (e) => {
    ...
    if (key === "username") {
      if (!usernameRegExp.test(e.target.value)) {
        setMessage({
          ...message,
          idMessage: "4~12자의 영문 대 소문자, 숫자만 사용 가능합니다",
        });
    ...
    if (key === "phone_number") {
      if (!phonNumberRegExp.test(e.target.value)) {
        setMessage({
          ...message,
          phonNumberMessage: '"-" 하이픈 없이 번호만 입력해주세요.',
        });
  ...
  const handleOnBlurId = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_URL}/users/checkId`,
  ...
  const handleSignup = () => {
    ...
    if (
      ...
      phonNumberRegExp.test(phone_number) &&
      address
    ) {
        ...
        <Wrapper onClick={(e) => e.stopPropagation()}>
          <SignUpForm onSubmit={(e) => e.preventDefault()}>
            ...
            <InputFieldDiv>
              <InputField
                placeholder="아이디"
                onBlur={(e) => handleOnBlurId(e)}
                onChange={handleInputValue("username")}
              />
              {userInfo.username.length > 0 && validation.idValidation ? (
                <Err>{message.idMessage}</Err>
              ) : null}
            </InputFieldDiv>
            <InputFieldDiv>
              <InputField
              ...
              />
              {userInfo.password.length > 0 && validation.passwordValidation ? (
                <Err>{message.passwordMessage}</Err>
              ) : null}
            </InputFieldDiv>
            <InputFieldDiv>
              <InputField
            ...
            <InputFieldDiv>
              ...
              validation.phonNumberValidation ? (
                <Err>{message.phonNumberMessage}</Err>
              ) : null}
            </InputFieldDiv>
            {visible ? (
              <>
            ...
            <InputFieldDiv>
              ...
              />
              {userInfo.nickname.length > 0 && validation.nicknameValidation ? (
                <Err>{message.nicknameMessage}</Err>
              ) : null}
            </InputFieldDiv>
            <ErrSignUp>{message.errorMessage}</ErrSignUp>
            <SignUpButton
              type="submit"
            ...
            <SignUpToLogin onClick={handleLogin}>
              로그인으로 돌아가기
            </SignUpToLogin>
          </SignUpForm>
        </Wrapper>
